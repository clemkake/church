 ...
 *
 *
 * @method     ChildUserConfigQuery orderByPersonId($order = Criteria::ASC) Order by the ucfg_per_id column
 ...
 *
 * @method     ChildUserConfigQuery groupByPersonId() Group by the ucfg_per_id column
 ...
 *
 * @method     ChildUserConfig findOneByPersonId(int $ucfg_per_id) Return the first ChildUserConfig filtered by the ucfg_per_id column
 ...
 *
 * @method     ChildUserConfig requireOneByPersonId(int $ucfg_per_id) Return the first ChildUserConfig filtered by the ucfg_per_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildUserConfig[]|ObjectCollection findByPersonId(int $ucfg_per_id) Return ChildUserConfig objects filtered by the ucfg_per_id column
...
abstract class UserConfigQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \EcclesiaCRM\Base\UserConfigQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\EcclesiaCRM\\UserConfig', $modelAlias = null)
    {
     ...
     * Example usage:
     * <code>
     * $query->filterByPersonId(1234); // WHERE ucfg_per_id = 1234
     * $query->filterByPersonId(array(12, 34)); // WHERE ucfg_per_id IN (12, 34)
     * $query->filterByPersonId(array('min' => 12)); // WHERE ucfg_per_id > 12
     * </code>
     *
     * @see       filterByUser()
     *
    ...
    public function filterByPersonId($personId = null, $comparison = null)
    {
        if (is_array($personId)) {
            ...
            if (isset($personId['min'])) {
                $this->addUsingAlias(UserConfigTableMap::COL_UCFG_PER_ID, $personId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($personId['max'])) {
                $this->addUsingAlias(UserConfigTableMap::COL_UCFG_PER_ID, $personId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserConfigTableMap::COL_UCFG_PER_ID, $personId, $comparison);
    }
     ...
     *
     * @param \EcclesiaCRM\User|ObjectCollection $user The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($userConfig) {
            $this->addCond('pruneCond0', $this->getAliasedColName(UserConfigTableMap::COL_UCFG_PER_ID), $userConfig->getPersonId(), Criteria::NOT_EQUAL);
