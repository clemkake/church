...
abstract class Deposit implements ActiveRecordInterface
{
    ...
    /**
     * The value for the dep_fund field.
     *
    ...
    /**
     * Get the [dep_fund] column value.
     *
     * @return int
     */
    public function getFund()
    {
        return $this->dep_fund;
    }
    ...
    /**
     * Set the value of [dep_fund] column.
     *
     * @param int $v new value
     * @return $this|\EcclesiaCRM\Deposit The current object (for fluent API support)
     */
    public function setFund($v)
    {
        ...
        if ($this->dep_fund !== $v) {
            $this->dep_fund = $v;
            $this->modifiedColumns[DepositTableMap::COL_DEP_FUND] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : DepositTableMap::translateFieldName('Fund', TableMap::TYPE_PHPNAME, $indexType)];
            $this->dep_fund = (null !== $col) ? (int) $col : null;
            $this->resetModified();
    ...
    {
        if ($this->aDonationFund !== null && $this->dep_fund !== $this->aDonationFund->getId()) {
            $this->aDonationFund = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(DepositTableMap::COL_DEP_FUND)) {
            $modifiedColumns[':p' . $index++]  = 'dep_Fund';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'dep_Type':
                        ...
                        break;
                    case 'dep_Fund':
                        $stmt->bindValue($identifier, $this->dep_fund, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(DepositTableMap::COL_DEP_FUND)) {
            $criteria->add(DepositTableMap::COL_DEP_FUND, $this->dep_fund);
        }
    ...
    {
        if ($this->aDonationFund === null && ($this->dep_fund != 0)) {
            $this->aDonationFund = ChildDonationFundQuery::create()->findPk($this->dep_fund, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aDonationFund->addDeposits($this);
             */
        }
    ...
    {
        if (null !== $this->aDonationFund) {
            $this->aDonationFund->removeDeposit($this);
        }
        $this->dep_id = null;
        $this->dep_date = null;
