...
abstract class EventCounts implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\EcclesiaCRM\\Map\\EventCountsTableMap';
    ...
    /**
     * @var        ChildEventCountName
     */
    protected $aEventCountName;
    ...
    /**
     * Initializes internal state of EcclesiaCRM\Base\EventCounts object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\EcclesiaCRM\EventCounts The current object (for fluent API support)
     */
    ...
    public function setEvtcntCountid($v)
    {
        ...
        if ($this->aEventCountName !== null && $this->aEventCountName->getId() !== $v) {
            $this->aEventCountName = null;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\EcclesiaCRM\\EventCounts'), 0, $e);
        }
    }
    ...
    {
        if ($this->aEvent !== null && $this->evtcnt_eventid !== $this->aEvent->getId()) {
            $this->aEvent = null;
        }
        if ($this->aEventCountName !== null && $this->evtcnt_countid !== $this->aEventCountName->getId()) {
            $this->aEventCountName = null;
        }
    } // ensureConsistency
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aEventCountName !== null) {
                if ($this->aEventCountName->isModified() || $this->aEventCountName->isNew()) {
                    $affectedRows += $this->aEventCountName->save($con);
                }
                $this->setEventCountName($this->aEventCountName);
            }
            ...
            if (null !== $this->aEventCountName) {
                ...
                switch ($keyType) {
                    ...
                    case TableMap::TYPE_FIELDNAME:
                        $key = 'eventcountnames_evctnm';
                        break;
                    default:
                        $key = 'EventCountName';
                }

                $result[$key] = $this->aEventCountName->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
            if (null !== $this->aEvent) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\EcclesiaCRM\EventCounts
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\EcclesiaCRM\EventCounts The current object, for fluid interface
     */
    ...
    public function hashCode()
    {
        ...
        //relation fk_evtcnt_countid to table eventcountnames_evctnm
        if ($this->aEventCountName && $hash = spl_object_hash($this->aEventCountName)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }

        //relation fk_evtcnt_event_ID to table events_event
     ...
     * objects.
     *
     * @param      object $copyObj An object of \EcclesiaCRM\EventCounts (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \EcclesiaCRM\EventCounts Clone of current object.
     * @throws PropelException
     */
    ...
    /**
     * Declares an association between this object and a ChildEventCountName object.
     *
     * @param  ChildEventCountName $v
     * @return $this|\EcclesiaCRM\EventCounts The current object (for fluent API support)
     * @throws PropelException
     */
    public function setEventCountName(ChildEventCountName $v = null)
    {
        if ($v === null) {
            $this->setEvtcntCountid(0);
        } else {
            $this->setEvtcntCountid($v->getId());
        }

        $this->aEventCountName = $v;

        // Add binding for other direction of this n:n relationship.
        // If this object has already been added to the ChildEventCountName object, it will not be re-added.
        if ($v !== null) {
            $v->addEventCounts($this);
        }
    ...
    /**
     * Get the associated ChildEventCountName object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildEventCountName The associated ChildEventCountName object.
     * @throws PropelException
     */
    public function getEventCountName(ConnectionInterface $con = null)
    {
        if ($this->aEventCountName === null && ($this->evtcnt_countid != 0)) {
            $this->aEventCountName = ChildEventCountNameQuery::create()->findPk($this->evtcnt_countid, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aEventCountName->addEventCountss($this);
             */
        }

        return $this->aEventCountName;
    }
    ...
    {
        if ($v === null) {
            $this->setEvtcntEventid(0);
        } else {
            $this->setEvtcntEventid($v->getId());
        }
    ...
    {
        if ($this->aEvent === null && ($this->evtcnt_eventid != 0)) {
            $this->aEvent = ChildEventQuery::create()->findPk($this->evtcnt_eventid, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aEvent->addEventCountss($this);
             */
        }
    ...
    {
        if (null !== $this->aEventCountName) {
            $this->aEventCountName->removeEventCounts($this);
        }
        if (null !== $this->aEvent) {
            $this->aEvent->removeEventCounts($this);
        }
