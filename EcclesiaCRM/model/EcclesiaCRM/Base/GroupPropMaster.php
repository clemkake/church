...
abstract class GroupPropMaster implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\EcclesiaCRM\\Map\\GroupPropMasterTableMap';
    ...
    /**
     * The value for the grp_mster_id field.
     *
     * @var        int
     */
    protected $grp_mster_id;
    ...
    /**
     * Initializes internal state of EcclesiaCRM\Base\GroupPropMaster object.
     * @see applyDefaults()
     */
    ...
    /**
     * Get the [grp_mster_id] column value.
     *
     * @return int
     */
    public function getId()
    {
        return $this->grp_mster_id;
    }
    ...
    /**
     * Set the value of [grp_mster_id] column.
     *
     * @param int $v new value
     * @return $this|\EcclesiaCRM\GroupPropMaster The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->grp_mster_id !== $v) {
            $this->grp_mster_id = $v;
            $this->modifiedColumns[GroupPropMasterTableMap::COL_GRP_MSTER_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $this->grp_mster_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : GroupPropMasterTableMap::translateFieldName('GroupId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->grp_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : GroupPropMasterTableMap::translateFieldName('PropId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->prop_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : GroupPropMasterTableMap::translateFieldName('Field', TableMap::TYPE_PHPNAME, $indexType)];
            $this->prop_field = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : GroupPropMasterTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->prop_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : GroupPropMasterTableMap::translateFieldName('Description', TableMap::TYPE_PHPNAME, $indexType)];
            $this->prop_description = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : GroupPropMasterTableMap::translateFieldName('TypeId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->type_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 7 + $startcol : GroupPropMasterTableMap::translateFieldName('Special', TableMap::TYPE_PHPNAME, $indexType)];
            $this->prop_special = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 8 + $startcol : GroupPropMasterTableMap::translateFieldName('PersonDisplay', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 9; // 9 = GroupPropMasterTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\EcclesiaCRM\\GroupPropMaster'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[GroupPropMasterTableMap::COL_GRP_MSTER_ID] = true;
        if (null !== $this->grp_mster_id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . GroupPropMasterTableMap::COL_GRP_MSTER_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(GroupPropMasterTableMap::COL_GRP_MSTER_ID)) {
            $modifiedColumns[':p' . $index++]  = 'grp_mster_id';
        }
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'grp_mster_id':
                        $stmt->bindValue($identifier, $this->grp_mster_id, PDO::PARAM_INT);
                        break;
                    case 'grp_ID':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array())
    {
        ...
        $result = array(
            ...
            $keys[2] => $this->getPropId(),
            $keys[3] => $this->getField(),
            $keys[4] => $this->getName(),
            $keys[5] => $this->getDescription(),
            $keys[6] => $this->getTypeId(),
            $keys[7] => $this->getSpecial(),
            $keys[8] => $this->getPersonDisplay(),
        );
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\EcclesiaCRM\GroupPropMaster
     */
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setPropId($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setField($arr[$keys[3]]);
        }
        ...
        if (array_key_exists($keys[7], $arr)) {
            $this->setSpecial($arr[$keys[7]]);
        }
        if (array_key_exists($keys[8], $arr)) {
            $this->setPersonDisplay($arr[$keys[8]]);
        }
    }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\EcclesiaCRM\GroupPropMaster The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(GroupPropMasterTableMap::COL_GRP_MSTER_ID)) {
            $criteria->add(GroupPropMasterTableMap::COL_GRP_MSTER_ID, $this->grp_mster_id);
        }
        if ($this->isColumnModified(GroupPropMasterTableMap::COL_GRP_ID)) {
    ...
    {
        $criteria = ChildGroupPropMasterQuery::create();
        $criteria->add(GroupPropMasterTableMap::COL_GRP_MSTER_ID, $this->grp_mster_id);
    ...
    /**
     * Generic method to set the primary key (grp_mster_id column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \EcclesiaCRM\GroupPropMaster (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \EcclesiaCRM\GroupPropMaster Clone of current object.
     * @throws PropelException
     */
