...
use \Exception;
use \PDO;
use EcclesiaCRM\Property as ChildProperty;
use EcclesiaCRM\PropertyQuery as ChildPropertyQuery;
 ...
 * @method     ChildPropertyQuery orderByProComment($order = Criteria::ASC) Order by the pro_Comment column
 *
 ...
 * @method     ChildPropertyQuery groupByProComment() Group by the pro_Comment column
 *
 ...
 *
 * @method     \EcclesiaCRM\PropertyTypeQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildProperty findOneByProComment(string $pro_Comment) Return the first ChildProperty filtered by the pro_Comment column *
 ...
 * @method     ChildProperty requireOneByProComment(string $pro_Comment) Return the first ChildProperty filtered by the pro_Comment column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildProperty[]|ObjectCollection findByProComment(string $pro_Comment) Return ChildProperty objects filtered by the pro_Comment column
...
abstract class PropertyQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \EcclesiaCRM\Base\PropertyQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\EcclesiaCRM\\Property', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT pro_ID, pro_Class, pro_prt_ID, pro_Name, pro_Description, pro_Prompt, pro_Comment FROM property_pro WHERE pro_ID = :p0';
        try {
    ...
    /**
     * Filter the query on the pro_Comment column
     *
     * Example usage:
     * <code>
     * $query->filterByProComment('fooValue');   // WHERE pro_Comment = 'fooValue'
     * $query->filterByProComment('%fooValue%', Criteria::LIKE); // WHERE pro_Comment LIKE '%fooValue%'
     * </code>
     *
     * @param     string $proComment The value to use as filter.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this|ChildPropertyQuery The current query, for fluid interface
     */
    public function filterByProComment($proComment = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($proComment)) {
                $comparison = Criteria::IN;
            }
        }

        return $this->addUsingAlias(PropertyTableMap::COL_PRO_COMMENT, $proComment, $comparison);
    }

    /**
     * Filter the query by a related \EcclesiaCRM\PropertyType object
     *
     * @param \EcclesiaCRM\PropertyType|ObjectCollection $propertyType The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    {
        if ($propertyType instanceof \EcclesiaCRM\PropertyType) {
            return $this
            ...
            throw new PropelException('filterByPropertyType() only accepts arguments of type \EcclesiaCRM\PropertyType or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \EcclesiaCRM\PropertyTypeQuery A secondary query class using the current class as primary query
     */
    public function usePropertyTypeQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'PropertyType', '\EcclesiaCRM\PropertyTypeQuery');
    }
