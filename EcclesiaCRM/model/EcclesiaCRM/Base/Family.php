...
abstract class Family implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\EcclesiaCRM\\Map\\FamilyTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildEgive[] Collection to store aggregation of ChildEgive objects.
     */
    protected $collEgives;
    protected $collEgivesPartial;
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildEgive[]
     */
    protected $egivesScheduledForDeletion = null;
    ...
    /**
     * Initializes internal state of EcclesiaCRM\Base\Family object.
     * @see applyDefaults()
     */
     ...
     * @return $this|\EcclesiaCRM\Family The current object (for fluent API support)
     */
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\EcclesiaCRM\\Family'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->peopleScheduledForDeletion !== null) {
                if (!$this->peopleScheduledForDeletion->isEmpty()) {
                    \EcclesiaCRM\PersonQuery::create()
            ...
            if ($this->egivesScheduledForDeletion !== null) {
                if (!$this->egivesScheduledForDeletion->isEmpty()) {
                    \EcclesiaCRM\EgiveQuery::create()
                        ->filterByPrimaryKeys($this->egivesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->egivesScheduledForDeletion = null;
                }
            }

            if ($this->collEgives !== null) {
                foreach ($this->collEgives as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collEgives) {
                ...
                $result[$key] = $this->collEgives->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\EcclesiaCRM\Family
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\EcclesiaCRM\Family The current object, for fluid interface
     */
     ...
     * objects.
     *
     * @param      object $copyObj An object of \EcclesiaCRM\Family (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getEgives() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addEgive($relObj->copy($deepCopy));
                }
            }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \EcclesiaCRM\Family Clone of current object.
     * @throws PropelException
     */
    ...
    public function initPeople($overrideExisting = true)
    {
        ...
        $this->collPeople->setModel('\EcclesiaCRM\Person');
    }
    ...
    public function setNoteShares(Collection $noteShares, ConnectionInterface $con = null)
    {
        ...
        foreach ($noteSharesToDelete as $noteShareRemoved) {
            $noteShareRemoved->setFamily(null);
        }
    ...
    public function setAutoPayments(Collection $autoPayments, ConnectionInterface $con = null)
    {
        ...
        foreach ($autoPaymentsToDelete as $autoPaymentRemoved) {
            $autoPaymentRemoved->setFamily(null);
        }
    ...
    /**
     * Clears out the collEgives collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     ...
     * @see        addEgives()
     */
    public function clearEgives()
    {
        $this->collEgives = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collEgives collection loaded partially.
     */
    public function resetPartialEgives($v = true)
    {
        $this->collEgivesPartial = $v;
    }

    /**
     * Initializes the collEgives collection.
     *
     * By default this just sets the collEgives collection to an empty array (like clearcollEgives());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param      boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initEgives($overrideExisting = true)
    {
        if (null !== $this->collEgives && !$overrideExisting) {
            return;
        }

        $collectionClassName = EgiveTableMap::getTableMap()->getCollectionClassName();

        $this->collEgives = new $collectionClassName;
        $this->collEgives->setModel('\EcclesiaCRM\Egive');
    }

    /**
     * Gets an array of ChildEgive objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildEgive[] List of ChildEgive objects
     * @throws PropelException
     */
    public function getEgives(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collEgivesPartial && !$this->isNew();
        if (null === $this->collEgives || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collEgives) {
                // return empty collection
                $this->initEgives();
            } else {
                $collEgives = ChildEgiveQuery::create(null, $criteria)
                    ->filterByFamily($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collEgivesPartial && count($collEgives)) {
                        $this->initEgives(false);

                        foreach ($collEgives as $obj) {
                            if (false == $this->collEgives->contains($obj)) {
                                $this->collEgives->append($obj);
                            }
                        }

                        $this->collEgivesPartial = true;
                    }

                    return $collEgives;
                }

                if ($partial && $this->collEgives) {
                    foreach ($this->collEgives as $obj) {
                        if ($obj->isNew()) {
                            $collEgives[] = $obj;
                        }
                    }
                ...
                $this->collEgives = $collEgives;
                $this->collEgivesPartial = false;
            }
        }

        return $this->collEgives;
    }

    /**
     * Sets a collection of ChildEgive objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $egives A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildFamily The current object (for fluent API support)
     */
    public function setEgives(Collection $egives, ConnectionInterface $con = null)
    {
        /** @var ChildEgive[] $egivesToDelete */
        $egivesToDelete = $this->getEgives(new Criteria(), $con)->diff($egives);


        $this->egivesScheduledForDeletion = $egivesToDelete;

        foreach ($egivesToDelete as $egiveRemoved) {
            $egiveRemoved->setFamily(null);
        }
    ...
    /**
     * Returns the number of related Egive objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Egive objects.
     * @throws PropelException
     */
    public function countEgives(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collEgivesPartial && !$this->isNew();
        if (null === $this->collEgives || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collEgives) {
            ...
            if ($partial && !$criteria) {
                return count($this->getEgives());
            }

            $query = ChildEgiveQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collEgives);
    }

    /**
     * Method called to associate a ChildEgive object to this object
     * through the ChildEgive foreign key attribute.
     *
     * @param  ChildEgive $l ChildEgive
     * @return $this|\EcclesiaCRM\Family The current object (for fluent API support)
     */
    public function addEgive(ChildEgive $l)
    {
        if ($this->collEgives === null) {
            $this->initEgives();
            $this->collEgivesPartial = true;
        ...
        if (!$this->collEgives->contains($l)) {
            $this->doAddEgive($l);

            if ($this->egivesScheduledForDeletion and $this->egivesScheduledForDeletion->contains($l)) {
                $this->egivesScheduledForDeletion->remove($this->egivesScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildEgive $egive The ChildEgive object to add.
     */
    protected function doAddEgive(ChildEgive $egive)
    {
        $this->collEgives[]= $egive;
        $egive->setFamily($this);
    }

    /**
     * @param  ChildEgive $egive The ChildEgive object to remove.
     * @return $this|ChildFamily The current object (for fluent API support)
     */
    public function removeEgive(ChildEgive $egive)
    {
        if ($this->getEgives()->contains($egive)) {
            $pos = $this->collEgives->search($egive);
            $this->collEgives->remove($pos);
            if (null === $this->egivesScheduledForDeletion) {
                $this->egivesScheduledForDeletion = clone $this->collEgives;
                $this->egivesScheduledForDeletion->clear();
            }
            $this->egivesScheduledForDeletion[]= clone $egive;
            $egive->setFamily(null);
        }
