...
abstract class Egive implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\EcclesiaCRM\\Map\\EgiveTableMap';
    ...
    /**
     * The value for the egv_id field.
     *
     * @var        int
     */
    protected $egv_id;
    ...
    /**
     * Initializes internal state of EcclesiaCRM\Base\Egive object.
     * @see applyDefaults()
     */
    ...
    /**
     * Set the value of [egv_id] column.
     *
     * @param int $v new value
     * @return $this|\EcclesiaCRM\Egive The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->egv_id !== $v) {
            $this->egv_id = $v;
            $this->modifiedColumns[EgiveTableMap::COL_EGV_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : EgiveTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->egv_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : EgiveTableMap::translateFieldName('EgiveId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->egv_egiveid = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : EgiveTableMap::translateFieldName('FamId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->egv_famid = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : EgiveTableMap::translateFieldName('DateEntered', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->egv_dateentered = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : EgiveTableMap::translateFieldName('DateLastEdited', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->egv_datelastedited = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : EgiveTableMap::translateFieldName('EnteredBy', TableMap::TYPE_PHPNAME, $indexType)];
            $this->egv_enteredby = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : EgiveTableMap::translateFieldName('EditedBy', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 7; // 7 = EgiveTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\EcclesiaCRM\\Egive'), 0, $e);
        }
    }
    ...
    {
        if ($this->aFamily !== null && $this->egv_famid !== $this->aFamily->getId()) {
            $this->aFamily = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[EgiveTableMap::COL_EGV_ID] = true;
        if (null !== $this->egv_id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . EgiveTableMap::COL_EGV_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(EgiveTableMap::COL_EGV_ID)) {
            $modifiedColumns[':p' . $index++]  = 'egv_ID';
        }
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'egv_ID':
                        $stmt->bindValue($identifier, $this->egv_id, PDO::PARAM_INT);
                        break;
                    case 'egv_egiveID':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getEgiveId(),
            $keys[2] => $this->getFamId(),
            $keys[3] => $this->getDateEntered(),
            $keys[4] => $this->getDateLastEdited(),
            $keys[5] => $this->getEnteredBy(),
            $keys[6] => $this->getEditedBy(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setEgiveId($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setFamId($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setDateEntered($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setDateLastEdited($arr[$keys[4]]);
        }
        if (array_key_exists($keys[5], $arr)) {
            $this->setEnteredBy($arr[$keys[5]]);
        }
        if (array_key_exists($keys[6], $arr)) {
            $this->setEditedBy($arr[$keys[6]]);
        }
    }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\EcclesiaCRM\Egive The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(EgiveTableMap::COL_EGV_ID)) {
            $criteria->add(EgiveTableMap::COL_EGV_ID, $this->egv_id);
        }
        if ($this->isColumnModified(EgiveTableMap::COL_EGV_EGIVEID)) {
    ...
    {
        $criteria = ChildEgiveQuery::create();
        $criteria->add(EgiveTableMap::COL_EGV_ID, $this->egv_id);
    ...
    /**
     * Generic method to set the primary key (egv_id column).
     *
     ...
     * objects.
     *
     * @param      object $copyObj An object of \EcclesiaCRM\Egive (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \EcclesiaCRM\Egive Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ($this->aFamily === null && ($this->egv_famid != 0)) {
            $this->aFamily = ChildFamilyQuery::create()->findPk($this->egv_famid, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aFamily->addEgives($this);
             */
        }
