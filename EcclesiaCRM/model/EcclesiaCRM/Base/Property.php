...
use EcclesiaCRM\PropertyType as ChildPropertyType;
use EcclesiaCRM\PropertyTypeQuery as ChildPropertyTypeQuery;
...
abstract class Property implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\EcclesiaCRM\\Map\\PropertyTableMap';
    ...
    /**
     * The value for the pro_comment field.
     * for GDPR
     * @var        string
     */
    protected $pro_comment;
    ...
    /**
     * Initializes internal state of EcclesiaCRM\Base\Property object.
     * @see applyDefaults()
     */
    ...
    /**
     * Get the [pro_comment] column value.
     * for GDPR
     * @return string
     */
    public function getProComment()
    {
        return $this->pro_comment;
    }
     ...
     * @return $this|\EcclesiaCRM\Property The current object (for fluent API support)
     */
    ...
    /**
     * Set the value of [pro_comment] column.
     * for GDPR
     * @param string $v new value
     * @return $this|\EcclesiaCRM\Property The current object (for fluent API support)
     */
    public function setProComment($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->pro_comment !== $v) {
            $this->pro_comment = $v;
            $this->modifiedColumns[PropertyTableMap::COL_PRO_COMMENT] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : PropertyTableMap::translateFieldName('ProComment', TableMap::TYPE_PHPNAME, $indexType)];
            $this->pro_comment = (null !== $col) ? (string) $col : null;
            $this->resetModified();
            ...
            return $startcol + 7; // 7 = PropertyTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\EcclesiaCRM\\Property'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PropertyTableMap::COL_PRO_COMMENT)) {
            $modifiedColumns[':p' . $index++]  = 'pro_Comment';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'pro_Prompt':
                        ...
                        break;
                    case 'pro_Comment':
                        $stmt->bindValue($identifier, $this->pro_comment, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\EcclesiaCRM\Property
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\EcclesiaCRM\Property The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PropertyTableMap::COL_PRO_COMMENT)) {
            $criteria->add(PropertyTableMap::COL_PRO_COMMENT, $this->pro_comment);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \EcclesiaCRM\Property (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setProComment($this->getProComment());
        if ($makeNew) {
            $copyObj->setNew(true);
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \EcclesiaCRM\Property Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ($this->aPropertyType === null && ($this->pro_prt_id != 0)) {
