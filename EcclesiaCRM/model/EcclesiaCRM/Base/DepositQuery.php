 ...
 * @method     ChildDepositQuery orderByFund($order = Criteria::ASC) Order by the dep_Fund column
 *
 ...
 * @method     ChildDepositQuery groupByFund() Group by the dep_Fund column
 *
 ...
 *
 * @method     ChildDepositQuery leftJoinDonationFund($relationAlias = null) Adds a LEFT JOIN clause to the query using the DonationFund relation
 * @method     ChildDepositQuery rightJoinDonationFund($relationAlias = null) Adds a RIGHT JOIN clause to the query using the DonationFund relation
 * @method     ChildDepositQuery innerJoinDonationFund($relationAlias = null) Adds a INNER JOIN clause to the query using the DonationFund relation
 *
 * @method     ChildDepositQuery joinWithDonationFund($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the DonationFund relation
 *
 * @method     ChildDepositQuery leftJoinWithDonationFund() Adds a LEFT JOIN clause and with to the query using the DonationFund relation
 * @method     ChildDepositQuery rightJoinWithDonationFund() Adds a RIGHT JOIN clause and with to the query using the DonationFund relation
 * @method     ChildDepositQuery innerJoinWithDonationFund() Adds a INNER JOIN clause and with to the query using the DonationFund relation
 *
 ...
 *
 * @method     \EcclesiaCRM\DonationFundQuery|\EcclesiaCRM\PledgeQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
 ...
 * @method     ChildDeposit findOneByFund(int $dep_Fund) Return the first ChildDeposit filtered by the dep_Fund column *
 ...
 * @method     ChildDeposit requireOneByFund(int $dep_Fund) Return the first ChildDeposit filtered by the dep_Fund column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildDeposit[]|ObjectCollection findByFund(int $dep_Fund) Return ChildDeposit objects filtered by the dep_Fund column
...
abstract class DepositQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \EcclesiaCRM\Base\DepositQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\EcclesiaCRM\\Deposit', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT dep_ID, dep_Date, dep_Comment, dep_EnteredBy, dep_Closed, dep_Type, dep_Fund FROM deposit_dep WHERE dep_ID = :p0';
        try {
    ...
    /**
     * Filter the query on the dep_Fund column
     *
     * Example usage:
     * <code>
     * $query->filterByFund(1234); // WHERE dep_Fund = 1234
     * $query->filterByFund(array(12, 34)); // WHERE dep_Fund IN (12, 34)
     * $query->filterByFund(array('min' => 12)); // WHERE dep_Fund > 12
     * </code>
     *
    ...
    public function filterByFund($fund = null, $comparison = null)
    {
        if (is_array($fund)) {
            $useMinMax = false;
            if (isset($fund['min'])) {
                $this->addUsingAlias(DepositTableMap::COL_DEP_FUND, $fund['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($fund['max'])) {
                $this->addUsingAlias(DepositTableMap::COL_DEP_FUND, $fund['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(DepositTableMap::COL_DEP_FUND, $fund, $comparison);
    }
    ...
    public function filterByDonationFund($donationFund, $comparison = null)
    {
        if ($donationFund instanceof \EcclesiaCRM\DonationFund) {
            return $this
                ->addUsingAlias(DepositTableMap::COL_DEP_FUND, $donationFund->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(DepositTableMap::COL_DEP_FUND, $donationFund->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
