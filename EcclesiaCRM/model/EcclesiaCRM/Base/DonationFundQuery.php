 ...
 *
 * @method     ChildDonationFundQuery leftJoinAutoPayment($relationAlias = null) Adds a LEFT JOIN clause to the query using the AutoPayment relation
 * @method     ChildDonationFundQuery rightJoinAutoPayment($relationAlias = null) Adds a RIGHT JOIN clause to the query using the AutoPayment relation
 * @method     ChildDonationFundQuery innerJoinAutoPayment($relationAlias = null) Adds a INNER JOIN clause to the query using the AutoPayment relation
 *
 * @method     ChildDonationFundQuery joinWithAutoPayment($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the AutoPayment relation
 *
 * @method     ChildDonationFundQuery leftJoinWithAutoPayment() Adds a LEFT JOIN clause and with to the query using the AutoPayment relation
 * @method     ChildDonationFundQuery rightJoinWithAutoPayment() Adds a RIGHT JOIN clause and with to the query using the AutoPayment relation
 * @method     ChildDonationFundQuery innerJoinWithAutoPayment() Adds a INNER JOIN clause and with to the query using the AutoPayment relation
 *
 * @method     ChildDonationFundQuery leftJoinDeposit($relationAlias = null) Adds a LEFT JOIN clause to the query using the Deposit relation
 * @method     ChildDonationFundQuery rightJoinDeposit($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Deposit relation
 * @method     ChildDonationFundQuery innerJoinDeposit($relationAlias = null) Adds a INNER JOIN clause to the query using the Deposit relation
 *
 * @method     ChildDonationFundQuery joinWithDeposit($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Deposit relation
 *
 * @method     ChildDonationFundQuery leftJoinWithDeposit() Adds a LEFT JOIN clause and with to the query using the Deposit relation
 * @method     ChildDonationFundQuery rightJoinWithDeposit() Adds a RIGHT JOIN clause and with to the query using the Deposit relation
 * @method     ChildDonationFundQuery innerJoinWithDeposit() Adds a INNER JOIN clause and with to the query using the Deposit relation
 *
 ...
 *
 * @method     \EcclesiaCRM\AutoPaymentQuery|\EcclesiaCRM\DepositQuery|\EcclesiaCRM\PledgeQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class DonationFundQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \EcclesiaCRM\Base\DonationFundQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\EcclesiaCRM\\DonationFund', $modelAlias = null)
    {
    ...
    public function filterByAutoPayment($autoPayment, $comparison = null)
    {
        if ($autoPayment instanceof \EcclesiaCRM\AutoPayment) {
            return $this
                ->addUsingAlias(DonationFundTableMap::COL_FUN_ID, $autoPayment->getFund(), $comparison);
    ...
    /**
     * Filter the query by a related \EcclesiaCRM\Deposit object
     *
     * @param \EcclesiaCRM\Deposit|ObjectCollection $deposit the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildDonationFundQuery The current query, for fluid interface
     */
    ...
    {
        if ($deposit instanceof \EcclesiaCRM\Deposit) {
            return $this
                ->addUsingAlias(DonationFundTableMap::COL_FUN_ID, $deposit->getFund(), $comparison);
        } elseif ($deposit instanceof ObjectCollection) {
            return $this
                ->useDepositQuery()
                ->filterByPrimaryKeys($deposit->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByDeposit() only accepts arguments of type \EcclesiaCRM\Deposit or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \EcclesiaCRM\DepositQuery A secondary query class using the current class as primary query
     */
    public function useDepositQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinDeposit($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Deposit', '\EcclesiaCRM\DepositQuery');
    }
