...
class UserConfigTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'EcclesiaCRM.Map.UserConfigTableMap';
     ...
     */
    const OM_CLASS = '\\EcclesiaCRM\\UserConfig';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'EcclesiaCRM.UserConfig';
    ...
    protected static $fieldNames = array (
        self::TYPE_PHPNAME       => array('PersonId', 'Id', 'Name', 'Value', 'Type', 'Tooltip', 'Permission', 'Cat', ),
        self::TYPE_CAMELNAME     => array('personId', 'id', 'name', 'value', 'type', 'tooltip', 'permission', 'cat', ),
    ...
    protected static $fieldKeys = array (
        self::TYPE_PHPNAME       => array('PersonId' => 0, 'Id' => 1, 'Name' => 2, 'Value' => 3, 'Type' => 4, 'Tooltip' => 5, 'Permission' => 6, 'Cat' => 7, ),
        self::TYPE_CAMELNAME     => array('personId' => 0, 'id' => 1, 'name' => 2, 'value' => 3, 'type' => 4, 'tooltip' => 5, 'permission' => 6, 'cat' => 7, ),
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\EcclesiaCRM\\UserConfig');
        $this->setPackage('EcclesiaCRM');
        $this->setUseIdGenerator(false);
        // columns
        $this->addForeignPrimaryKey('ucfg_per_id', 'PersonId', 'SMALLINT' , 'user_usr', 'usr_per_ID', true, 9, null);
    ...
    {
        $this->addRelation('User', '\\EcclesiaCRM\\User', RelationMap::MANY_TO_ONE, array (
  0 =>
  array (
    0 => ':ucfg_per_id',
    1 => ':usr_per_ID',
     ...
     * and findPk*() calls.
     *
     * @param \EcclesiaCRM\UserConfig $obj A \EcclesiaCRM\UserConfig object.
    ...
    public static function addInstanceToPool($obj, $key = null)
    {
        if (Propel::isInstancePoolingEnabled()) {
            if (null === $key) {
                $key = serialize([(null === $obj->getPersonId() || is_scalar($obj->getPersonId()) || is_callable([$obj->getPersonId(), '__toString']) ? (string) $obj->getPersonId() : $obj->getPersonId()), (null === $obj->getId() || is_scalar($obj->getId()) || is_callable([$obj->getId(), '__toString']) ? (string) $obj->getId() : $obj->getId())]);
            } // if key === null
     ...
     *
     * @param mixed $value A \EcclesiaCRM\UserConfig object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \EcclesiaCRM\UserConfig) {
                $key = serialize([(null === $value->getPersonId() || is_scalar($value->getPersonId()) || is_callable([$value->getPersonId(), '__toString']) ? (string) $value->getPersonId() : $value->getPersonId()), (null === $value->getId() || is_scalar($value->getId()) || is_callable([$value->getId(), '__toString']) ? (string) $value->getId() : $value->getId())]);
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \EcclesiaCRM\UserConfig object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if ($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PersonId', TableMap::TYPE_PHPNAME, $indexType)] === null && $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)] === null) {
            return null;
        }

        return serialize([(null === $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PersonId', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PersonId', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PersonId', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PersonId', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('PersonId', TableMap::TYPE_PHPNAME, $indexType)]), (null === $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)])]);
    }
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \EcclesiaCRM\UserConfig) { // it's a model object
